...
compare-brave-just-like-the-firefox-browser = Just like the { -brand-name-firefox } browser, the { -brand-name-brave } browser is free, open source and focused on protecting users’ privacy. { -brand-name-brave } is a relative newcomer to the world of browsers: its maker, Brave Software, first debuted the browser in January 2016. In this article we’ll compare our { -brand-name-firefox } browser with the { -brand-name-brave } browser in three categories: privacy, utility and portability.

# "Chromium" is a brand name. To "piggy-back" means to ride on someone's back or shoulders, and here means that open source software builds upon the work of others.
# Variables:
...
compare-brave-the-brave-browser-like-so = The { -brand-name-brave } browser, like so many others, is built on Google’s open-source Chromium code. Open-source means anyone can use the source code and piggy-back on top of it to build whatever they want — like the <a { $opera }>{ -brand-name-opera }</a> and <a { $edge }>{ -brand-name-edge }</a> browsers. But it doesn’t mean that all Chromium-based browsers are equal or are themselves open source.

# "Chromium" is a brand name.
compare-brave-brave-differentiates-itself = { -brand-name-brave } differentiates itself from the other Chromium browsers by focusing on user privacy — specifically by blocking trackers, scripts, and ads by default. So when you use the { -brand-name-brave } browser, the areas of a website that would normally display ads appear as blank spaces. In some instances, pages don’t load properly, which will require you to either choose a different browser or flip the ‘Shields Up’ setting to ‘Shields Down’ which turns off the privacy and security protection.

# "Basic Attention Token" and "BAT" are brand names. "a.k.a." is an abbreviation of "also known as".
compare-brave-the-internet-as-a-whole-is = The Internet as a whole is largely paid for by display advertisements, which keeps the actual content you want to view free. { -brand-name-brave } has attempted to upend this model by encouraging its users to opt into Brave’s own reward system, which in reality, is its own ad platform. Once a user has opted-in, { -brand-name-brave } will display what they call “privacy-respecting ads” for which you can view and earn what they call a Basic Attention Token a.k.a. a BAT. From this point { -brand-name-brave } users can choose to spend their BATs on supporting the sites or individual contributors they love, who in turn can convert the BATs into actual currency.
...
compare-brave-brave-also-supports-google = { -brand-name-brave } also supports Google Chrome’s huge library of extensions available in the { -brand-name-chrome } web store and offers a variety of in-browser features like the aforementioned { -brand-name-brave } Rewards program, and support for downloading torrents in the browser.
